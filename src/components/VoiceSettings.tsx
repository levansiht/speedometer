...
export function VoiceSettings({onClose}: VoiceSettingsProps = {}) {
  ...
  const handleTestVoice = useCallback(async () => {
    await voiceService.speak(
      'Xin chào! Đây là giọng đọc tiếng Việt của ứng dụng đo tốc độ.',
    );
  }, []);
      ...
      <ScrollView style={styles.content}>
        <View style={styles.section}>
          <View style={styles.settingRow}>
            ...
            <Switch
              ...
              thumbColor={
                config.enabled ? colors.background : colors.textSecondary
              }
            />
          </View>
        </View>
            ...
            {[1, 5, 10].map(interval => (
              <TouchableOpacity
                ...
                <Text
                  variant="button"
                  color={
                    config.intervalKm === interval ? 'inverse' : 'primary'
                  }>
                  {interval} km
                </Text>
              </TouchableOpacity>
        ...
        <View style={styles.section}>
          <Text variant="caption" color="secondary" style={styles.note}>
            💡 Tip: Giọng đọc sẽ tự động thông báo khi bắt đầu, tạm dừng, tiếp
            tục và kết thúc hành trình.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
