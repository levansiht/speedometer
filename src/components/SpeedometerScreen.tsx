...
export function SpeedometerScreen() {
  ...
  useEffect(() => {
    ...
    const handleTripEvents = async () => {
      if (
        currentTrip.status === TripStatus.RUNNING &&
        currentTrip.stats.distance === 0
      ) {
  ...
  const speedKMH = useMemo(
    () => convertSpeed(speedMS, SpeedUnit.KMH),
    [speedMS],
  );
  const speedMPH = useMemo(
    () => convertSpeed(speedMS, SpeedUnit.MPH),
    [speedMS],
  );
  ...
  const handleStopTrip = useCallback(async () => {
    Alert.alert(
      'Kết thúc chuyến đi',
      'Bạn có chắc muốn kết thúc chuyến đi này?',
      [
        {text: 'Hủy', style: 'cancel'},
        {
          text: 'Kết thúc',
          ...
          onPress: async () => {
            if (currentTrip) {
              await voiceService.announceTripEnd({
                distance: currentTrip.stats.distance,
                duration: currentTrip.stats.duration,
                avgSpeed: currentTrip.stats.averageSpeed,
              });
            }
            await stopTrip();
